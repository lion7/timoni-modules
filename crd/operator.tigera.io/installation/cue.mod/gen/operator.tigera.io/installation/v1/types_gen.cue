...
//timoni:generate timoni vendor crd -f https://raw.githubusercontent.com/projectcalico/calico/refs/heads/master/charts/tigera-operator/crds/operator.tigera.io_installations_crd.yaml
...
// Installation configures an installation of Calico or Calico
// Enterprise. At most one instance
// of this resource is supported. It must be named "default". The
// Installation API installs core networking
...
// Enterprise installation.
#InstallationSpec: {
	// CalicoKubeControllersDeployment configures the
	// calico-kube-controllers Deployment. If used in
	// conjunction with the deprecated ComponentResources, then these
	...
	calicoKubeControllersDeployment?: {
		...
		spec?: {
			...
			// be set on the calico-kube-controllers Deployment.
			// If omitted, the calico-kube-controllers Deployment will use its
			// default value for minReadySeconds.
			minReadySeconds?: int32 & >=0

			// Template describes the calico-kube-controllers Deployment pod
			...
			template?: {
				...
				spec?: {
					// Affinity is a group of affinity scheduling rules for the
					// calico-kube-controllers pods.
					...
					// calico-kube-controllers Deployment affinity.
					affinity?: {
					...
					// calico-kube-controllers Deployment containers.
					// If omitted, the calico-kube-controllers Deployment will use its
					// default values for its containers.
					containers?: [...{
						// Name is an enum which identifies the calico-kube-controllers
						// Deployment container by name.
						// Supported values are: calico-kube-controllers,
						// es-calico-kube-controllers
						name: "calico-kube-controllers" | "es-calico-kube-controllers"

						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named calico-kube-controllers
						// Deployment container's resources.
						// If omitted, the calico-kube-controllers Deployment will use its
					...
					// NodeSelector is the calico-kube-controllers pod's scheduling
					// constraints.
					// If specified, each of the key/value pairs are added to the
					// calico-kube-controllers Deployment nodeSelector provided
					// the key does not already exist in the object's nodeSelector.
					// If used in conjunction with ControlPlaneNodeSelector, that
					// nodeSelector is set on the calico-kube-controllers Deployment
					// and each of this field's key/value pairs are added to the
					// calico-kube-controllers Deployment nodeSelector provided
					// the key does not already exist in the object's nodeSelector.
					// If omitted, the calico-kube-controllers Deployment will use its
					...
					// WARNING: Please note that this field will override the default
					// calico-kube-controllers Deployment tolerations.
					tolerations?: [...{
	...
	// CalicoNetwork specifies networking configuration options for
	// Calico.
	calicoNetwork?: {
		// BGP configures whether or not to enable Calico's BGP
		// capabilities.
		bgp?: "Enabled" | "Disabled"

		// ContainerIPForwarding configures whether ip forwarding will be
		// enabled for containers in the CNI configuration.
		// Default: Disabled
		containerIPForwarding?: "Enabled" | "Disabled"

		// HostPorts configures whether or not Calico will support
		// Kubernetes HostPorts. Valid only when using the Calico CNI
		// plugin.
		// Default: Enabled
		hostPorts?: "Enabled" | "Disabled"

		// IPPools contains a list of IP pools to manage. If nil, a single
		// IPv4 IP pool
		// will be created by the operator. If an empty list is provided,
		// the operator will not create any IP pools and will instead
		// wait for IP pools to be created out-of-band.
		// IP pools in this list will be reconciled by the operator and
		// should not be modified out-of-band.
		ipPools?: list.MaxItems(25) & [...{
			// AllowedUse controls what the IP pool will be used for. If not
			// specified or empty, defaults to
			// ["Tunnel", "Workload"] for back-compatibility
			allowedUses?: [...string]

			// BlockSize specifies the CIDR prefex length to use when
			// allocating per-node IP blocks from
			// the main IP pool CIDR.
			// Default: 26 (IPv4), 122 (IPv6)
			blockSize?: int

			// CIDR contains the address range for the IP Pool in classless
			// inter-domain routing format.
			cidr: string

			// DisableBGPExport specifies whether routes from this IP pool's
			// CIDR are exported over BGP.
			// Default: false
			disableBGPExport?: bool | *false

			// DisableNewAllocations specifies whether or not new IP
			// allocations are allowed from this pool.
			// This is useful when you want to prevent new pods from receiving
			// IP addresses from this pool, without
			// impacting any existing pods that have already been assigned
			// addresses from this pool.
			disableNewAllocations?: bool

			// Encapsulation specifies the encapsulation type that will be
			// used with
			// the IP Pool.
			// Default: IPIP
			encapsulation?: "IPIPCrossSubnet" | "IPIP" | "VXLAN" | "VXLANCrossSubnet" | "None"

			// Name is the name of the IP pool. If omitted, this will be
			// generated.
			name?: string

			// NATOutgoing specifies if NAT will be enabled or disabled for
			// outgoing traffic.
			// Default: Enabled
			natOutgoing?: "Enabled" | "Disabled"

			// NodeSelector specifies the node selector that will be set for
			// the IP Pool.
			// Default: 'all()'
			nodeSelector?: string
		}]

		// LinuxDataplane is used to select the dataplane used for Linux
		// nodes. In particular, it
		// causes the operator to add required mounts and environment
		// variables for the particular dataplane.
		// If not specified, iptables mode is used.
		// Default: Iptables
		linuxDataplane?: "Iptables" | "BPF" | "VPP" | "Nftables"

		// LinuxPolicySetupTimeoutSeconds delays new pods from running
		// containers
		// until their policy has been programmed in the dataplane.
		// The specified delay defines the maximum amount of time
		// that the Calico CNI plugin will wait for policy to be
		// programmed.
		// Only applies to pods created on Linux nodes.
		// * A value of 0 disables pod startup delays.
		// Default: 0
		linuxPolicySetupTimeoutSeconds?: int

		// MTU specifies the maximum transmission unit to use on the pod
		// network.
		// If not specified, Calico will perform MTU auto-detection based
		// on the cluster network.
		mtu?: int

		// MultiInterfaceMode configures what will configure multiple
		// interface per pod. Only valid for Calico Enterprise
		// installations
		// using the Calico CNI plugin.
		// Default: None
		multiInterfaceMode?: "None" | "Multus"

		// NodeAddressAutodetectionV4 specifies an approach to
		// automatically detect node IPv4 addresses. If not specified,
		// will use default auto-detection settings to acquire an IPv4
		// address for each node.
		nodeAddressAutodetectionV4?: {
			// CanReach enables IP auto-detection based on which source
			// address on the node is used to reach the
			// specified IP or domain.
			canReach?: string

			// CIDRS enables IP auto-detection based on which addresses on the
			// nodes are within
			// one of the provided CIDRs.
			cidrs?: [...string]

			// FirstFound uses default interface matching parameters to select
			// an interface, performing best-effort
			// filtering based on well-known interface names.
			firstFound?: bool

			// Interface enables IP auto-detection based on interfaces that
			// match the given regex.
			interface?: string

			// Kubernetes configures Calico to detect node addresses based on
			// the Kubernetes API.
			kubernetes?: "NodeInternalIP"

			// SkipInterface enables IP auto-detection based on interfaces
			// that do not match
			// the given regex.
			skipInterface?: string
		}

		// NodeAddressAutodetectionV6 specifies an approach to
		// automatically detect node IPv6 addresses. If not specified,
		...
		sysctl?: [...{
			key:   "net.ipv4.tcp_keepalive_intvl" | "net.ipv4.tcp_keepalive_probes" | "net.ipv4.tcp_keepalive_time"
			value: string
		}]

		// WindowsDataplane is used to select the dataplane used for
		// Windows nodes. In particular, it
		// causes the operator to add required mounts and environment
		// variables for the particular dataplane.
		// If not specified, it is disabled and the operator will not
		// render the Calico Windows nodes daemonset.
		// Default: Disabled
		windowsDataplane?: "HNS" | "Disabled"
	}

	// CalicoNodeDaemonSet configures the calico-node DaemonSet. If
	// used in
	// conjunction with the deprecated ComponentResources, then these
	// overrides take precedence.
	calicoNodeDaemonSet?: {
		...
		spec?: {
			// MinReadySeconds is the minimum number of seconds for which a
			// newly created DaemonSet pod should
			// be ready without any of its container crashing, for it to be
			// considered available.
			// If specified, this overrides any minReadySeconds value that may
			// be set on the calico-node DaemonSet.
			// If omitted, the calico-node DaemonSet will use its default
			// value for minReadySeconds.
			minReadySeconds?: int32 & >=0

			// Template describes the calico-node DaemonSet pod that will be
			// created.
			template?: {
				// Metadata is a subset of a Kubernetes object's metadata that is
				// added to
				...
				spec?: {
					...
					containers?: [...{
						// Name is an enum which identifies the calico-node DaemonSet
						// container by name.
						...
						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named calico-node DaemonSet
					...
					initContainers?: [...{
						// Name is an enum which identifies the calico-node DaemonSet init
						// container by name.
						// Supported values are: install-cni, hostpath-init,
						// flexvol-driver, mount-bpffs, node-certs-key-cert-provisioner,
						// calico-node-prometheus-server-tls-key-cert-provisioner
						name: "install-cni" | "hostpath-init" | "flexvol-driver" | "mount-bpffs" | "node-certs-key-cert-provisioner" | "calico-node-prometheus-server-tls-key-cert-provisioner"
					...
					// WARNING: Please note that this field will modify the default
					// calico-node DaemonSet nodeSelector.
					nodeSelector?: {
						[string]: string
					}
	...
	calicoNodeWindowsDaemonSet?: {
		...
		spec?: {
			...
			// considered available.
			// If specified, this overrides any minReadySeconds value that may
			// be set on the calico-node-windows DaemonSet.
			// If omitted, the calico-node-windows DaemonSet will use its
			// default value for minReadySeconds.
			minReadySeconds?: int32 & >=0

			// Template describes the calico-node-windows DaemonSet pod that
			// will be created.
			template?: {
				// Metadata is a subset of a Kubernetes object's metadata that is
				// added to
				...
				spec?: {
					// Affinity is a group of affinity scheduling rules for the
					// calico-node-windows pods.
					// If specified, this overrides any affinity that may be set on
					// the calico-node-windows DaemonSet.
					// If omitted, the calico-node-windows DaemonSet will use its
					// default value for affinity.
					// WARNING: Please note that this field will override the default
					// calico-node-windows DaemonSet affinity.
					affinity?: {
						// Describes node affinity scheduling rules for the pod.
						nodeAffinity?: {
							// The scheduler will prefer to schedule pods to nodes that
					...
					// If specified, this overrides the specified calico-node-windows
					// DaemonSet containers.
					// If omitted, the calico-node-windows DaemonSet will use its
					// default values for its containers.
					containers?: [...{
						...
						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named calico-node-windows
						// DaemonSet container's resources.
						// If omitted, the calico-node-windows DaemonSet will use its
						// default value for this container's resources.
						// If used in conjunction with the deprecated ComponentResources,
						// then this value takes precedence.
					...
					// InitContainers is a list of calico-node-windows init
					// containers.
					// If specified, this overrides the specified calico-node-windows
					// DaemonSet init containers.
					// If omitted, the calico-node-windows DaemonSet will use its
					...
					initContainers?: [...{
						...
						name: "install-cni" | "hostpath-init" | "flexvol-driver" | "mount-bpffs" | "node-certs-key-cert-provisioner" | "calico-node-windows-prometheus-server-tls-key-cert-provisioner"
					...
					// NodeSelector is the calico-node-windows pod's scheduling
					// constraints.
					// If specified, each of the key/value pairs are added to the
	...
	// Deprecated. The CalicoWindowsUpgradeDaemonSet is deprecated and
	// will be removed from the API in the future.
	// CalicoWindowsUpgradeDaemonSet configures the
	// calico-windows-upgrade DaemonSet.
	calicoWindowsUpgradeDaemonSet?: {
		...
		spec?: {
			...
			// considered available.
			// If specified, this overrides any minReadySeconds value that may
			// be set on the calico-windows-upgrade DaemonSet.
			// If omitted, the calico-windows-upgrade DaemonSet will use its
			// default value for minReadySeconds.
			minReadySeconds?: int32 & >=0

			// Template describes the calico-windows-upgrade DaemonSet pod
			// that will be created.
			template?: {
				// Metadata is a subset of a Kubernetes object's metadata that is
				// added to
				...
				spec?: {
					// Affinity is a group of affinity scheduling rules for the
					// calico-windows-upgrade pods.
					// If specified, this overrides any affinity that may be set on
					// the calico-windows-upgrade DaemonSet.
					// If omitted, the calico-windows-upgrade DaemonSet will use its
					// default value for affinity.
					// WARNING: Please note that this field will override the default
					// calico-windows-upgrade DaemonSet affinity.
					affinity?: {
						// Describes node affinity scheduling rules for the pod.
						nodeAffinity?: {
							// The scheduler will prefer to schedule pods to nodes that
					...
					// If specified, this overrides the specified
					// calico-windows-upgrade DaemonSet containers.
					// If omitted, the calico-windows-upgrade DaemonSet will use its
					// default values for its containers.
					containers?: [...{
						// Name is an enum which identifies the calico-windows-upgrade
						// DaemonSet container by name.
						name: "calico-windows-upgrade"

						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named calico-windows-upgrade
						// DaemonSet container's resources.
						// If omitted, the calico-windows-upgrade DaemonSet will use its
						// default value for this container's resources.
						resources?: {
					...
					// NodeSelector is the calico-windows-upgrade pod's scheduling
					// constraints.
					// If specified, each of the key/value pairs are added to the
					// calico-windows-upgrade DaemonSet nodeSelector provided
					// the key does not already exist in the object's nodeSelector.
					// If omitted, the calico-windows-upgrade DaemonSet will use its
					// default value for nodeSelector.
					// WARNING: Please note that this field will modify the default
					...
					// default value for tolerations.
					// WARNING: Please note that this field will override the default
					// calico-windows-upgrade DaemonSet tolerations.
					tolerations?: [...{
						// Effect indicates the taint effect to match. Empty means match
						// all taint effects.
						// When specified, allowed values are NoSchedule, PreferNoSchedule
	...
	// API in order
	// to obtain TLS certificates. This feature requires that you
	// bring your own CSR signing and approval process, otherwise
	// pods will be stuck during initialization.
	certificateManagement?: {
		// Certificate of the authority that signs the
		// CertificateSigningRequests in PEM format.
		caCert: string

		// Specify the algorithm used by pods to generate a key pair that
		// is associated with the X.509 certificate request.
		// Default: RSAWithSize2048
		keyAlgorithm?: "" | "RSAWithSize2048" | "RSAWithSize4096" | "RSAWithSize8192" | "ECDSAWithCurve256" | "ECDSAWithCurve384" | "ECDSAWithCurve521"

		// Specify the algorithm used for the signature of the X.509
		// certificate request.
		// Default: SHA256WithRSA
		signatureAlgorithm?: "" | "SHA256WithRSA" | "SHA384WithRSA" | "SHA512WithRSA" | "ECDSAWithSHA256" | "ECDSAWithSHA384" | "ECDSAWithSHA512"

		// When a CSR is issued to the certificates.k8s.io API, the
		// signerName is added to the request in order to accommodate for
		// clusters
	...
	cni?: {
		ipam?: {
			// Specifies the IPAM plugin that will be used in the Calico or
			...
			// The IPAM plugin is installed and configured only if the CNI
			// plugin is set to Calico,
			// for all other values of the CNI plugin the plugin binaries and
			// CNI config is a dependency
			// that is expected to be installed separately.
			// Default: Calico
			type: "Calico" | "HostLocal" | "AmazonVPC" | "AzureVNET"
		}

		// Specifies the CNI plugin that will be used in the Calico or
		// Calico Enterprise installation.
		...
		// Installation resource is created, this field defaults to
		// AmazonVPC.
		// * For all other cases this field defaults to Calico.
		// For the value Calico, the CNI plugin binaries and CNI config
		// will be installed as part of deployment,
		// for all other values the CNI plugin binaries and CNI config is
		// a dependency that is expected
	...
	// and KubeControllersDeployment.
	// ComponentResources can be used to customize the resource
	// requirements for each component.
	// Node, Typha, and KubeControllers are supported for
	// installations.
	componentResources?: [...{
		// ComponentName is an enum which identifies the component
		componentName: "Node" | "Typha" | "KubeControllers"

		// ResourceRequirements allows customization of limits and
		// requests for compute resources such as cpu and memory.
	...
	// ControlPlaneNodeSelector is used to select control plane nodes
	// on which to run Calico
	// components. This is globally applied to all resources created
	// by the operator excluding daemonsets.
	controlPlaneNodeSelector?: {
		[string]: string
	}

	// ControlPlaneReplicas defines how many replicas of the control
	// plane core components will be deployed.
	// This field applies to all control plane components that support
	// High Availability. Defaults to 2.
	controlPlaneReplicas?: int

	// ControlPlaneTolerations specify tolerations which are then
	// globally applied to all resources
	...
	csiNodeDriverDaemonSet?: {
		...
		spec?: {
			...
			// considered available.
			// If specified, this overrides any minReadySeconds value that may
			// be set on the csi-node-driver DaemonSet.
			// If omitted, the csi-node-driver DaemonSet will use its default
			// value for minReadySeconds.
			minReadySeconds?: int32 & >=0

			// Template describes the csi-node-driver DaemonSet pod that will
			// be created.
			template?: {
				// Metadata is a subset of a Kubernetes object's metadata that is
				// added to
				...
				spec?: {
					// Affinity is a group of affinity scheduling rules for the
					// csi-node-driver pods.
					// If specified, this overrides any affinity that may be set on
					// the csi-node-driver DaemonSet.
					// If omitted, the csi-node-driver DaemonSet will use its default
					// value for affinity.
					// WARNING: Please note that this field will override the default
					// csi-node-driver DaemonSet affinity.
					affinity?: {
						// Describes node affinity scheduling rules for the pod.
						nodeAffinity?: {
							// The scheduler will prefer to schedule pods to nodes that
					...
					// If specified, this overrides the specified csi-node-driver
					// DaemonSet containers.
					// If omitted, the csi-node-driver DaemonSet will use its default
					// values for its containers.
					containers?: [...{
						// Name is an enum which identifies the csi-node-driver DaemonSet
						// container by name.
						// Supported values are: calico-csi, csi-node-driver-registrar.
						name: "calico-csi" | "csi-node-driver-registrar" | "csi-node-driver"

						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named csi-node-driver
						// DaemonSet container's resources.
						// If omitted, the csi-node-driver DaemonSet will use its default
						// value for this container's resources.
						resources?: {
					...
					// NodeSelector is the csi-node-driver pod's scheduling
					// constraints.
					// If specified, each of the key/value pairs are added to the
					...
					// value for nodeSelector.
					// WARNING: Please note that this field will modify the default
					// csi-node-driver DaemonSet nodeSelector.
					nodeSelector?: {
						[string]: string
					}
					...
					// value for tolerations.
					// WARNING: Please note that this field will override the default
					// csi-node-driver DaemonSet tolerations.
					tolerations?: [...{
						// Effect indicates the taint effect to match. Empty means match
						// all taint effects.
						// When specified, allowed values are NoSchedule, PreferNoSchedule
	...
	// FIPSMode uses images and features only that are using FIPS
	// 140-2 validated cryptographic modules and standards.
	// Only supported for Variant=Calico.
	// Default: Disabled
	fipsMode?: "Enabled" | "Disabled"

	// FlexVolumePath optionally specifies a custom path for
	// FlexVolume. If not specified, FlexVolume will be
	// enabled by default. If set to 'None', FlexVolume will be
	// disabled. The default is based on the
	// kubernetesProvider.
	flexVolumePath?: string

	// ImagePath allows for the path part of an image to be specified.
	// If specified
	// then the specified value will be used as the image path for
	// each image. If not specified
	...
	// ImagePrefix allows for the prefix part of an image to be
	// specified. If specified
	// then the given value will be used as a prefix on each image. If
	// not specified
	...
	// registry pull secrets to use. These are
	...
	// KubeletVolumePluginPath optionally specifies enablement of
	// Calico CSI plugin. If not specified,
	// CSI will be enabled by default. If set to 'None', CSI will be
	// disabled.
	// Default: /var/lib/kubelet
	kubeletVolumePluginPath?: string

	// KubernetesProvider specifies a particular provider of the
	// Kubernetes platform and enables provider-specific
	// configuration.
	// If the specified value is empty, the Operator will attempt to
	// automatically determine the current provider.
	// If the specified value is not empty, the Operator will still
	// attempt auto-detection, but
	// will additionally compare the auto-detected value to the
	// specified value to confirm they match.
	kubernetesProvider?: "" | "EKS" | "GKE" | "AKS" | "OpenShift" | "DockerEnterprise" | "RKE2" | "TKG"
	logging?: {
		...
		cni?: {
			...
			// Default: 100Mi
			logFileMaxSize?: (int | string) & {
				=~"^(\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))(([KMGTPE]i)|[numkMGTPE]|([eE](\\+|-)?(([0-9]+(\\.[0-9]*)?)|(\\.[0-9]+))))?$"
			}

			// Default: Info
			logSeverity?: "Error" | "Warning" | "Debug" | "Info"
		}
	}
	...
	// prometheus metrics may still be configured through
	// FelixConfiguration.
	nodeMetricsPort?: int

	// NodeUpdateStrategy can be used to customize the desired update
	// strategy, such as the MaxUnavailable
	// field.
	nodeUpdateStrategy?: {
	...
	// Docker images.
	// If specified then the given value must end with a slash
	// character (`/`) and all images will be pulled from this
	// registry.
	// If not specified then the default registries will be used. A
	// special case value, UseDefault, is
	// supported to explicitly specify the default registries will be
	// used.
	// Image format:
	// `<registry><imagePath>/<imagePrefix><imageName>:<image-tag>`
	...
	// Kubernetes Service CIDRs. Specifying this is required when
	// using Calico for Windows.
	serviceCIDRs?: [...string]
	typhaAffinity?: {
		// NodeAffinity describes node affinity scheduling rules for
		// typha.
		nodeAffinity?: {
			// The scheduler will prefer to schedule pods to nodes that
			// satisfy
	...
	// TyphaDeployment configures the typha Deployment. If used in
	// conjunction with the deprecated
	// ComponentResources or TyphaAffinity, then these overrides take
	// precedence.
	typhaDeployment?: {
		// Metadata is a subset of a Kubernetes object's metadata that is
		// added to the Deployment.
		...
		spec?: {
			...
			// considered available.
			// If specified, this overrides any minReadySeconds value that may
			// be set on the typha Deployment.
			// If omitted, the typha Deployment will use its default value for
			// minReadySeconds.
			minReadySeconds?: int32 & >=0
			strategy?: {
			...
			// Template describes the typha Deployment pod that will be
			// created.
			template?: {
				// Metadata is a subset of a Kubernetes object's metadata that is
				// added to
				...
				spec?: {
					...
					// pods.
					// If specified, this overrides any affinity that may be set on
					// the typha Deployment.
					// If omitted, the typha Deployment will use its default value for
					// affinity.
					// If used in conjunction with the deprecated TyphaAffinity, then
					...
					// If specified, this overrides the specified typha Deployment
					// containers.
					// If omitted, the typha Deployment will use its default values
					// for its containers.
					containers?: [...{
						// Name is an enum which identifies the typha Deployment container
						// by name.
						...
						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named typha Deployment
						// container's resources.
						// If omitted, the typha Deployment will use its default value for
					...
					initContainers?: [...{
						// Name is an enum which identifies the typha Deployment init
						// container by name.
						...
						// Resources allows customization of limits and requests for
						// compute resources such as cpu and memory.
						// If specified, this overrides the named typha Deployment init
						// container's resources.
						// If omitted, the typha Deployment will use its default value for
						// this init container's resources.
						// If used in conjunction with the deprecated ComponentResources,
						// then this value takes precedence.
						resources?: {
							// Claims lists the names of resources, defined in
					...
					// the key does not already exist in the object's nodeSelector.
					// If omitted, the calico-typha Deployment will use its default
					// value for nodeSelector.
					// WARNING: Please note that this field will modify the default
					// calico-typha Deployment nodeSelector.
					nodeSelector?: {
						[string]: string
					}
	...
	windowsNodes?: {
		// CNIBinDir is the path to the CNI binaries directory on Windows,
		// it must match what is used as 'bin_dir' under
		// [plugins]
		...
		// CNIConfigDir is the path to the CNI configuration directory on
		// Windows, it must match what is used as 'conf_dir' under
		// [plugins]
		// [plugins."io.containerd.grpc.v1.cri"]
		// [plugins."io.containerd.grpc.v1.cri".cni]
		// on the containerd 'config.toml' file on the Windows nodes.
		...
		// CNILogDir is the path to the Calico CNI logs directory on
		// Windows.
		cniLogDir?: string

		// VXLANAdapter is the Network Adapter used for VXLAN, leave blank
		// for primary NIC
		vxlanAdapter?: string

		// VXLANMACPrefix is the prefix used when generating MAC addresses
		// for virtual NICs
		vxlanMACPrefix?: =~"^[0-9A-Fa-f]{2}-[0-9A-Fa-f]{2}$"
	}
}
